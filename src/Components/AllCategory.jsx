...
const AllCategory = () => {
  ...
  useEffect(() => {
    fetch(
      `https://assigenment10.vercel.app/subItems?subcategory_name=${subCategory}`
    )
      .then((res) => res.json())
      .then((datas) => {
        console.log(datas);
  ...
  return (
    <div className="container mx-auto p-4 mb-28">
      <h1 className="text-3xl font-bold mt-8">
        All Craft & Art Related to{" "}
        <span className="text-orange-600">{subCategory}</span> Is Here :
      </h1>
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mt-12">
        {data.map((item) => (
          <SubCard key={item._id} item={item}></SubCard>
        ))}
      </div>
    </div>
  );
